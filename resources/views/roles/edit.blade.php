...
<div class="container">
  <div class="row"> 
    <div class="col-md-12">
      <h4>Modulo para la Asignación de Permisos a Roles</h4>
    </div>
  </div>
</div>   
  ...
  <div class="col-md-12">          
      {!! Form::model($rol, ['route' => ['roles.update',$rol->id]]) !!}
          <div class="row well"> 
           ...
           <div class="row well"> 
              <h4>Permisos - ><a type="button" class="btn btn-default btn-xs" alin="right" data-toggle="modal" data-target="#favoritesModal">Agregar</a></h4>
                @foreach ($rol->permisos as $permiso)
                  <label>{{$permiso->slug}}</label>
                  <a href="" type="button" class="btn btn-default btn-xs" data-toggle="modal" data-target="#eliminarPermiso" onclick="guardarRol({{$rol->id}}),guardarPermiso({{$permiso->id}})">Eliminar</a><br>
                @endforeach
           </div>

      {!! Form::hidden('id',$rol->id) !!}
      <a class="btn btn-info btn-sm" href="{{ route('roles.index') }}">Regresar</a>
      <a class="btn btn-success btn-sm" href="{{ route('roles.index') }}">Confirmar Acciones</a>
      {!! Form::close() !!}
  </div>
  </div>
</div>
